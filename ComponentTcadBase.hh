...
class ComponentTcadBase : public Component {
 public:
  ...
  /// Import mesh and field map from a Silvaco ATHENA (.str) or ATLAS (.sta) file.
  bool InitialiseSilvaco(const std::string& filename);
  ...
  bool GetElementNodes(const std::size_t i,
                       std::vector<std::size_t>& nodes, bool) const override;
  ...
  /// Get the number of defect states found in the map.
  std::size_t GetNumberOfDefects() { return m_defects.size(); }

  /** Set the properties of a defect state.
   * \param i index of the defect
   ...
   */
  bool SetDefect(const std::size_t i, const double exsec, const double hxsec,
                 const double concentration);
  /// List the properties of all traps in the map.
  void PrintDefects() const;
  ...
  /// Return the labels of electrodes/readouts defined for this component.
  std::vector<std::string> GetElectrodeLabels() const override;
  double GetElementLength(int i) { return m_elements[i].length; }

  std::array<double, N> GetVertex(int elem, int vert) {
    return m_vertices[m_elements[elem].vertex[vert]];
  }

  double GetElementType(int i) { return m_elements[i].type; }
 ...
 protected:
  ...
  struct Element {
    ...
    // Length of the elements minimal edge
    double length{0.};
  };
  ...
  struct WeightingFieldCopy {
    std::string source;  // label of the source electrode
    double rot[3][3];    // 3x3 rotation
    double trans[3];     // translation vector
  };
  std::unordered_map<std::string, WeightingFieldCopy> m_wfieldCopies;
  ...
  std::vector<std::vector<float>> m_defectOcc;
  // Attachment coefficients [1 / cm]
  std::vector<double> m_eEta;
  std::vector<double> m_hEta;

  enum class DefectType { Donor = 0, Acceptor };
  struct Defect {
    /// Defect type
    DefectType type;
  ...
  bool LoadSilvaco(const std::string& filename);
  bool ReadDataset(std::ifstream& datafile, const std::string& dataset);
  bool SetupDefects(const std::string& line);
